...
HashTable *hastable_merge(HashTable* h1, HashTable* h2);
...
HashTable *hastable_merge(HashTable* h1, HashTable* h2){
    HashTable* h3 = hashtable_create(h1->nbuckets + h2->nbuckets);

    for (size_t i = 0; i < h1->nbuckets; i++)
    {
        for (list* l = h1->bucket[i]; l->next != NULL; l = l->next)
        {
            hashtable_insert(h3,l->val.key, l->val.value);
        }
    }

    for (size_t i = 0; i < h2->nbuckets; i++)
    {
        for (list* l = h2->bucket[i]; l->next != NULL; l = l->next)
        {
            hashtable_insert(h3,l->val.key, l->val.value);
        }
    }
